...
namespace :system do
  ...
  desc "系统数据更新"
  task :data, [:method_name] => [:environment] do |t, args|
    unless args[:method_name].blank?
      Skyeye.safe_call args[:method_name]
    else
      puts "Please specify a method name like `rake system:data[create_default_data_source]` "
      exit(1)
    end
  end
